/**
 * HTTP client for the Python Feature Store endpoint (POST /features/compute).
 *
 * Mirrors the fail-closed conventions of dq/stage-b-client.ts:
 *  - explicit timeout; typed errors; no silent degradation.
 *  - 401/403 -> FeatureComputeAuthError (fail-fast; shared-secret misconfig).
 *  - any other non-2xx / transport / malformed body -> FeatureComputeError.
 *
 * Hash discipline: the response `featureHash` is read as an OPAQUE STRING and
 * `features` is captured exactly as decoded — this client performs NO hashing,
 * NO key reordering, and NO numeric normalization.
 */

import type { FeatureComputeInput, FeatureComputeResult } from "./types.js";
...
/**
 * Validate the decoded body WITHOUT mutating it. `features` and `featureHash`
 * are returned by reference / verbatim — no canonicalization happens here.
 */
function decode(raw: string): FeatureComputeResult {
  ...
  return {
    ...
    version,
    asOfTs,
    // OPAQUE pass-through — string and object handed on without transformation.
    featureHash,
