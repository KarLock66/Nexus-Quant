 ...
 * demo rows, then SIGKILL it mid-run. Restart it, wait until it completes at
...
export async function runPhase2(): Promise<void> {
  ...
  assert((await countDemoRows()) === 0, "expected 0 EngineSignal rows before worker boot");

  // ── Run #1: boot, let it persist the two rows, then crash it (SIGKILL). ──────
  const w1 = spawnWorker({ SIGNAL_TICK_MS: "1000" });
  try {
    await waitFor(
      "worker#1 persists the 2 demo rows",
      async () => (await countDemoRows()) === 2,
      { timeoutMs: 30_000 },
    );
  } finally {
