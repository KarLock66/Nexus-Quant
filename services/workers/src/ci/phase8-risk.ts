 ...
 * (paper-broker) market adapter over the deterministic fixture lineage, then proves the
 * Phase 8 guarantees end-to-end under runtime conditions:
 *
 *   1. every order routes through the risk engine: with generous limits the fixture
 ...
 *
 * Like Phase 6/7 this needs only the idempotent fixture upstream chain (no schema, no
...
export async function runPhase8(): Promise<void> {
  ...
  try {
    ...
    const res = await runSignalPipelineTick({ prisma, log: quiet, tickId: "phase8-a", execution: deps });
    const filled = res.execution?.filled ?? 0;
    assert(filled >= 1, "expected the fixture lineage to fill at least one risk-approved order");
    ...
    const tightAdapter = createMarketExecutionAdapter({ marketData: fixtureQuoteProvider() });
    ...
    const blockRes = await runSignalPipelineTick({ prisma, log: quiet, tickId: "phase8-b", execution: tightDeps });
    ...
    const haltedAdapter = createMarketExecutionAdapter({ marketData: fixtureQuoteProvider() });
    ...
    const haltedRes = await runSignalPipelineTick({ prisma, log: quiet, tickId: "phase8-c", execution: haltedDeps });
